...
/** Duration → lookback text for min_created_at instructions. */
const LOOKBACK_LABEL: Record<string, string> = {
  "24h": "24 hours ago",
  "7d": "7 days ago",
  "30d": "30 days ago",
};

export function registerMarketRoundupPrompt(server: McpServer) {
  server.registerPrompt(
    "market_roundup",
    {
      description:
        "Market roundup: priority-ranked briefing of top stories with smart account signals, mindshare movers, and catalysts over a user-selected time horizon.",
      argsSchema: {
        duration: z
          .enum(["24h", "7d", "30d"])
          .optional()
          .describe(
            "Time horizon for the roundup: 24h, 7d, 30d (default: 24h). If the chosen horizon yields insufficient data, expand to the next tier.",
          ),
      },
    },
    ({ duration }) => {
      const horizon = duration || "24h";
      const lookback = LOOKBACK_LABEL[horizon] ?? `${horizon} ago`;
        ...
        messages: [
          {
            ...
            content: {
              type: "text" as const,
              text: `Generate a market roundup for the past ${horizon} by gathering data from multiple sources. Follow these steps exactly — do not skip or shortcut any step.

**Batch 1 — Run these four calls in parallel (no dependencies):**

1. kaito_advanced_search with sort_by="smart_engagement", sources="Twitter", size=50, min_created_at set to ${lookback} in ISO 8601 (no tokens, no keyword — pure discovery mode).
2. kaito_market_smart_following with duration="${horizon}", user_tag_individual_or_organization="Organization", user_web3_relevance="relevant", filter_smart_followers_operator="lte", filter_smart_followers_value=100, sort_by="followers_change", sort_order="desc".
3. kaito_mindshare_delta with duration="${horizon}" for biggest mindshare gainers.
4. kaito_market_smart_following with duration="${horizon}", user_tag_individual_or_organization="Individual", user_web3_relevance="relevant", sort_by="followers_change", sort_order="desc".
...
7. From Step 4 results, pick the top 5 individuals by followers_change. Run ONE search: kaito_advanced_search with usernames="{handle1},{handle2},{handle3},{handle4},{handle5}" (comma-separated), sources="Twitter", size=50, min_created_at set to ${lookback} — what they've been saying that's driving new smart followers.
...
Start with an **Answer** section: 2–3 paragraphs that directly tell the reader what happened in the past ${horizon} and what matters most. This is the PRIMARY output — write as if the reader will only read this section. Lead with the single biggest story and its implications, then cover 2–3 secondary themes, weaving in smart account signals and data as supporting evidence. End with what to watch next.
